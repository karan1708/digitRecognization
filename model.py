...
model = Sequential()
# Adding convolution layer with 32 filters to extract the features
model.add(Conv2D(32, kernel_size=(3, 3),activation='relu',input_shape=input_shape))

# Adding convolution layer with 64 filters to extract the features from the previous layers
model.add(Conv2D(64, (3, 3), activation='relu'))
# reduces the size of the feature map resulting in memory and computation optimization
model.add(MaxPool2D(pool_size=(2, 2)))

# To prevent  our model from learning from redundant details
model.add(Dropout(0.25))
model.add(Flatten())
model.add(Dense(128, activation='relu'))
model.add(Dropout(0.5))

# normalizing our probability values
